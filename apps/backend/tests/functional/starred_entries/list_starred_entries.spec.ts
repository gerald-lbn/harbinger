...
test.group('List starred entries', (group) => {
  group.each.setup(() => testUtils.db().truncate())

  test('returns 200 with an empty array if there are no records', async ({ client }) => {
    const user = await UserFactory.create()

    const response = await client.get('/api/v1/starred_entries').loginAs(user)
  ...
  test('returns 401 if user is not authenticated', async ({ client }) => {
    const response = await client.get('/api/v1/starred_entries')
